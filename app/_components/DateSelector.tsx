...
import { Cabin, Settings } from "@/app/_ts/interfaces/app_interfaces";
...
function DateSelector({ settings, cabin, bookedDates }: DateSelectorProps) {
  ...
  if (displayedRange?.to === undefined || displayedRange?.from === undefined) {
    numNights = 0;
  } else {
    numNights = differenceInDays(displayedRange.to, displayedRange.from);
  }
